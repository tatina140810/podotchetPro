...
FEATURE_FLAGS: list[FlagDef] = [
    ...
    FlagDef(
        key="workspace_auditor_access",
        label="Аудитору видна детализация пространств",
        description=(
            "Открывает ролям admin и auditor полную детализацию расходов проектных "
            "пространств (категории, описания, чеки). Выключено — они видят только "
            "агрегат по владельцу (получено/потрачено/остаток). Журнал действий с "
            "пространствами ведётся всегда, независимо от этого тумблера."
        ),
        default=False,
        group="Проектные пространства",
    ),
]
